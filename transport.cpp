...
void Transport::ReceiveAudioFrame(const QByteArray &audio_byte)
{
    ...
    if (audio_frame_buffer_.size() <
        current_buffer_index_ + FRAME_LEN)  // 缓冲区不足以播放一帧的音频
    {
        return;
    }
    else
    {
        ...
        audio_frame.append(
            &audio_frame_buffer_.data()[current_buffer_index_], FRAME_LEN);
        emit ReceiveReady(audio_frame);
        current_buffer_index_ += FRAME_LEN;

        if (current_buffer_index_ > MAX_BUFFER_LEN)
        {
            audio_frame_buffer_ = audio_frame_buffer_.right(
                audio_frame_buffer_.size() - MAX_BUFFER_LEN);
